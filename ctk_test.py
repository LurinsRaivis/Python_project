...
ctk.set_default_color_theme("blue")  # Options include "blue" (default), "green", "dark-blue", etc.
...
info_label = ctk.CTkLabel(info_frame, height=100, width=400, text="", wraplength=400)
...
def process_and_plot_data(df, data_type, info_label, start_date, end_date):
    ...
    if time_diff <= 35:
        df_agg = df.groupby(df['date'].dt.date).agg({data_type: 'mean'})
        plot_title = "Daily"
    elif time_diff <= 180:
        df_agg = df.groupby(df['date'].dt.to_period('W').dt.start_time).agg({data_type: 'mean'})
        plot_title = "Weekly"
    elif time_diff <= 365:
        df_agg = df.groupby(df['date'].dt.to_period('M').dt.start_time).agg({data_type: 'mean'})
        plot_title = "Monthly"
    else:
        df_agg = df.groupby(df['date'].dt.to_period('Y').dt.start_time).agg({data_type: 'mean'})
        plot_title = "Yearly"
    ...
    if data_type == 'temperature_2m':
        ...
        # Update info_label
        hottest_coldest_text =  f"Hottest {period_label}: {hottest_period} - Average temperature was {hottest_temp:.2f}°C\n" \
                                f"Coldest {period_label}: {coldest_period} - Average temperature was {coldest_temp:.2f}°C"
        info_label.configure(text=hottest_coldest_text)
