...
def fetch_data():
    ...
    try:
        ...
        logging.info(f"Data successfully fetched for {city}")

        process_and_plot_data(df, data_type, info_text, start_date, end_date)
...
def download_csv():
    ...
    # Make sure 'df' is accessible here; you might need to store it as a global variable
    global df
    if df is not None:
...
graph_frame = tk.Frame(app, width=1344)
controls_frame = tk.Frame(app, width=576) 
info_frame = tk.Frame(graph_frame, width=1344, height=100)  # Adjust the size as needed

info_text = tk.Text(info_frame, height=4, width=50, bg='#ffffff', fg='#000000', borderwidth=2, relief="solid")
...
city_choice = ttk.Combobox(controls_frame, textvariable=city_var, values=list(cities.keys()), state='readonly')
...
data_type_choice = ttk.Combobox(controls_frame, textvariable=data_type_var, values=data_types, state='readonly')
data_type_choice.pack(pady=10)
line_chart_rb = tk.Radiobutton(controls_frame, text="Line Chart", variable=chart_type_var, value="line")
bar_chart_rb = tk.Radiobutton(controls_frame, text="Bar Chart", variable=chart_type_var, value="bar")
line_chart_rb.pack(in_=controls_frame, pady=5)
bar_chart_rb.pack(in_=controls_frame, pady=5)
fetch_button = tk.Button(controls_frame, text="Fetch Data", command=fetch_data)
fetch_button.pack(pady=10)
output_label = tk.Label(controls_frame, text="")
output_label.pack(pady=10)
download_button = tk.Button(controls_frame, text="Download CSV", command=download_csv)
download_button.pack(pady=10)
quit_button = tk.Button(controls_frame, text="Quit", command=app.quit)
quit_button.pack(pady=10)

def process_and_plot_data(df, data_type, info_text, start_date, end_date):
    global canvas_widget
    ...
    # Plotting
    if chart_type_var.get() == "line":
        df_agg[data_type].plot(kind='line', ax=ax, label=f"{data_type} Data")
    elif chart_type_var.get() == "bar":
        df_agg[data_type].plot(kind='bar', ax=ax, label=f"{data_type} Data")
    ax.set_xlabel('Date')
    ...
    # Update info_text with additional information
    ...
    if data_type == 'temperature_2m':
        ...
        # Update info_text
        info_text.delete('1.0', tk.END)
        info_text.insert(tk.END, f"Hottest {period_label}: {hottest_period} - Average temperature was {hottest_temp:.2f}°C\n")
        info_text.insert(tk.END, f"Coldest {period_label}: {coldest_period} - Average temperature was {coldest_temp:.2f}°C\n")
